...
function filteraddding(item1, item2) {
  const filteringspace = document.getElementById("Filters-id");
  filteringspace.style.marginTop = 8`px`;
...
function makingpagination(items) {
  ...
  if (toalnumberofpagination > 0) {
    ...
    item4.innerHTML = `Showing ${1} – ${items.length} of ${
      items.length
    } results for "Mobiles"`;
...
function callingmakingproduct(pageNumber) {
  ...
  if (pageNumber === 1) {
    ...
    item4.innerHTML = `Showing ${
      lengthresult - reminder
    } – ${lengthresult} of ${lengthresult} results for "mobiles"`;
  } else {
    itemsToDisplay = therealresult.slice(startIndex, startIndex + pageSize);
    item4.innerHTML = `Showing ${startIndex + 1} – ${
      startIndex + pageSize
    } of ${lengthresult} results for "mobiles"`;
  }
